...
struct DataSender {
    ...
    void start(const std::string& networkAddress,
        const std::string& filterName = ""); // address following zmq conventions, e.g. "tcp://localhost:1234"
    void stop();

    bool sendData(std::string const& v);
    bool sendData(std::string const& filterName, std::string const& v);

    template <typename DataType> bool sendData(std::string const& filterName, DataType const& v);
...
struct CameraConfiguration {
    ...
    // the matrices may be from the VR SDK and thus better suited for VR stereo rendering than the explicit camera
    // parameters above
    mat4 viewMatrix;
...
struct BoundingBoxCorners {
    ...
    vec4 getCenteringVector() const { return -1.0f * (min + ((max - min) * 0.5f)); }

    // returns transformation matrix to center bounding box (and data set) at (0,0,0)
    // the convention between Unity and our renderer is that the data set is centered at (and can be rotated around)
    // zero
    ...
    BoundingBoxCorners getCenteredBoundingBox() const {
        ...
        return BoundingBoxCorners{this->min + transform.data[3], this->max + transform.data[3]};
    }
};
...
#define make_sendData(DataTypeName)                                                                                    \
    template <> bool interop::DataSender::sendData<DataTypeName>(std::string const& filterName, DataTypeName const& v);
