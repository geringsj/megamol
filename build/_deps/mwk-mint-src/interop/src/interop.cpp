...
static const void restorePreviousFbo(interop::glFramebuffer* fbo) {
    ...
    glViewport(
        fbo->m_previousViewport[0], fbo->m_previousViewport[1], fbo->m_previousViewport[2], fbo->m_previousViewport[3]);

    fbo->m_previousFramebuffer[0] = 0;
    fbo->m_previousFramebuffer[1] = 0;
    for (int i = 0; i < 4; i++) fbo->m_previousViewport[i] = 0;
}
...
void interop::glFramebuffer::init(uint width, uint height) {
    ...
    // im so sorry
    if (!glDrawBuffersEXT) loadGlExtensions();

    m_width = width;
    m_height = height;

    if (!m_glFbo) glGenFramebuffersEXT(1, &m_glFbo);

    if (!m_glTextureRGBA8) glGenTextures(1, &m_glTextureRGBA8);
    glBindTexture(GL_TEXTURE_2D, m_glTextureRGBA8);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, m_width, m_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

    if (!m_glTextureDepth) glGenTextures(1, &m_glTextureDepth);
    ...
    if (e != GL_FRAMEBUFFER_COMPLETE_EXT) printf("There is a problem with the FBO\n");

    restorePreviousFbo(this);
}

void interop::glFramebuffer::destroy() {
    if (m_glFbo) glDeleteFramebuffersEXT(1, &m_glFbo);

    if (m_glTextureRGBA8) glDeleteTextures(1, &m_glTextureRGBA8);

    if (m_glTextureDepth) glDeleteTextures(1, &m_glTextureDepth);
...
void interop::glFramebuffer::blitTexture() {
    ...
    glBlitFramebufferEXT(0, 0, m_width, m_height, m_previousViewport[0], m_previousViewport[1], m_previousViewport[2],
        m_previousViewport[3], GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT, GL_NEAREST);
...
interop::TextureSender::TextureSender() { m_sender = std::make_shared<SpoutSender>(); }
...
void interop::TextureSender::init(std::string name, uint width, uint height) {
    if (name.size() == 0 || name.size() > 256) return;
...
void interop::DataSender::stop() {
    if (m_socket->connected()) m_socket->close();
}

bool interop::DataSender::sendData(std::string const& v) { return this->sendData(m_filterName, v); }
...
namespace {
void runThread_DataReceiver(
    interop::DataReceiver* dr, const std::string& networkAddress, const std::string& filterName) {
    ...
    try {
        ...
        socket.setsockopt(
            ZMQ_SUBSCRIBE, filter.data(), filter.size()); // only receive messages with prefix given by filterName
    ...
    while (threadRunning) {
        ...
        if (filterEmpty || more) {
            if (!socket.recv(&content_msg)) continue;
        } else
            continue;
        ...
        if (log) {
            std::cout << "zmq received address: "
                      << std::string(static_cast<char*>(address_msg.data()), address_msg.size()) << std::endl;
            std::cout << "zmq received content: "
                      << std::string(static_cast<char*>(content_msg.data()), content_msg.size()) << std::endl;
        }

        // store message content for user to retrieve+parse
        std::lock_guard<std::mutex> lock{mutex};
        msgData.assign(static_cast<char*>(content_msg.data()), content_msg.size());
        newDataFlag.clear(std::memory_order_release); // Sets false. All writes in the current thread are visible in
...
bool interop::DataReceiver::getDataCopy() {
    if (!m_newDataFlag.test_and_set(
            std::memory_order_acquire)) { // Sets true, returns previous value. All writes in other threads that release
...
void to_json(json& j, const vec4& v) { j = json{writeVal(x), writeVal(y), writeVal(z), writeVal(w)}; }
...
void to_json(json& j, const mat4& v) {
    j = json{{"e00", v.data[0].x}, {"e01", v.data[1].x}, {"e02", v.data[2].x}, {"e03", v.data[3].x},
        {"e10", v.data[0].y}, {"e11", v.data[1].y}, {"e12", v.data[2].y}, {"e13", v.data[3].y}, {"e20", v.data[0].z},
        {"e21", v.data[1].z}, {"e22", v.data[2].z}, {"e23", v.data[3].z}, {"e30", v.data[0].w}, {"e31", v.data[1].w},
        {"e32", v.data[2].w}, {"e33", v.data[3].w}};
}
...
void to_json(json& j, const CameraView& v) { j = json{writeVal(eyePos), writeVal(lookAtPos), writeVal(camUpDir)}; }
...
void to_json(json& j, const StereoCameraView& v) { j = json{writeVal(leftEyeView), writeVal(rightEyeView)}; }
...
void to_json(json& j, const CameraProjection& v) {
    j = json{writeVal(fieldOfViewY_rad), writeVal(nearClipPlane), writeVal(farClipPlane), writeVal(aspect),
...
void to_json(json& j, const CameraConfiguration& v) {
    j = json{
        writeVal(viewParameters), writeVal(projectionParameters), writeVal(viewMatrix), writeVal(projectionMatrix)};
}
...
void to_json(json& j, const StereoCameraConfiguration& v) {
    j = json{
        writeVal(stereoConvergence), writeVal(stereoSeparation), writeVal(cameraLeftEye), writeVal(cameraRightEye)};
}
...
void to_json(json& j, const BoundingBoxCorners& v) { j = json{writeVal(min), writeVal(max)}; }
...
void to_json(json& j, const ModelPose& v) {
    j = json{writeVal(translation), writeVal(scale), writeVal(rotation_axis_angle_rad), writeVal(modelMatrix)};
}
...
void to_json(json& j, const ParameterFloat& v) { j = json{ writeVal(param), writeVal(name), writeVal(modulFullName) }; }

void from_json(const json& j, ParameterFloat& v) {
	readVal(param);
	readVal(name);
	readVal(modulFullName);
}

void to_json(json& j, const ParameterEnum& v) { j = json{ writeVal(param), writeVal(name), writeVal(modulFullName) }; }

void from_json(const json& j, ParameterEnum& v) {
	readVal(param);
	readVal(name);
	readVal(modulFullName);
}

void to_json(json& j, const ParameterVec3& v) { j = json{ writeVal(param), writeVal(name), writeVal(modulFullName) }; }

void from_json(const json& j, ParameterVec3& v) {
	readVal(param);
	readVal(name);
	readVal(modulFullName);
}
...
#define make_dataGet(DataTypeName)                                                                                     \
    template <> bool interop::DataReceiver::getData<DataTypeName>(DataTypeName & v) {                                  \
        if (this->getDataCopy()) {                                                                                     \
            auto& byteData = m_msgDataCopy;                                                                            \
            if (byteData.size()) {                                                                                     \
...
// when adding to those macros, don't forget to also define 'to_json' and 'from_json' functions above for the new data
// type
make_dataGet(interop::BoundingBoxCorners) make_dataGet(interop::DatasetRenderConfiguration)
    make_dataGet(interop::ModelPose) make_dataGet(interop::StereoCameraConfiguration)
        make_dataGet(interop::CameraConfiguration) make_dataGet(interop::CameraProjection)
            make_dataGet(interop::StereoCameraView) make_dataGet(interop::CameraView) make_dataGet(interop::mat4)
                make_dataGet(interop::vec4) make_dataGet(interop::VisBool) make_dataGet(interop::ParameterInt)
                    make_dataGet(interop::ParameterBool) make_dataGet(interop::ParameterFloat) make_dataGet(interop::ParameterEnum)
	make_dataGet(interop::ParameterVec3)
#undef make_dataGet
...
#define make_sendData(DataTypeName)                                                                                    \
    ...
    // when adding to those macros, don't forget to also define 'to_json' and 'from_json' functions above for the new
    // data type
    make_sendData(interop::BoundingBoxCorners) make_sendData(interop::DatasetRenderConfiguration)
        make_sendData(interop::ModelPose) make_sendData(interop::StereoCameraConfiguration)
            make_sendData(interop::CameraConfiguration) make_sendData(interop::CameraProjection)
                make_sendData(interop::StereoCameraView) make_sendData(interop::CameraView) make_sendData(interop::mat4)
                    make_sendData(interop::vec4) make_sendData(interop::VisBool) make_sendData(interop::ParameterInt)
                        make_sendData(interop::ParameterFloat) make_sendData(interop::ParameterEnum) make_sendData(interop::ParameterBool)
	make_sendData(interop::ParameterVec3)
#undef make_sendData
...
interop::vec4 interop::operator*(const float s, const interop::vec4 v) { return v * s; }