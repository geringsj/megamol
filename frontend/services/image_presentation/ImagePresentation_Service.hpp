...
private:
    ...
    using ImageEntry = std::pair<std::string, GenericImage>;
    std::list<ImageEntry> m_images;

    megamol::frontend_resources::GenericImageRegistry m_image_registry_resource; // resource to expose requesting/renaming/deleting images
    ...
    using EntryPointExecutionCallback =
        std::function<bool(void*, std::vector<megamol::frontend::FrontendResource> const&, GenericImage&)>;
    ...
    struct GraphEntryPoint {
        ...
        std::reference_wrapper<GenericImage> execution_result_image;
    };
