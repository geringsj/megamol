...
bool ImagePresentation_Service::init(const Config& config) {
    ...
    m_image_registry_resource.pimpl = &m_images; // pimpl access happens in frontend resources GenericImage.cpp
    ...
    {
          {"GenericImageRegistry", m_image_registry_resource} // mostly we use this resource, but other services like Screenshots may access it too
    ...
    this->m_requestedResourcesNames =
    {
        ...
        , "GenericImageToPNG_ScreenshotTrigger"
    };
...
using EntryPointExecutionCallback = std::function<bool(void*, std::vector<FrontendResource> const&, GenericImage&)>;
...
void ImagePresentation_Service::register_lua_framebuffer_callbacks() {
    ...
    callbacks.add<VoidResult, std::string>(
        ...
        {
            auto maybe_triggerscreenshot_ptr = maybeGetResource<std::function<bool(GenericImage const&, std::string const&)>>("GenericImageToPNG_ScreenshotTrigger", m_frontend_resources);
